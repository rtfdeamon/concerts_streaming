...
export default function page() {
  const shows = useAppSelector(state => state.shows.events).filter(show => new Date(show.date) > new Date(Date.now()));  useEffect(() => {
    const today = new Date();
    const lastWeek = new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000);
    dispatch(getShowByFilter({to: getIsoStringDate(today.toLocaleString('en-US', CSTTimeZoneOptions)), from: getIsoStringDate(lastWeek.toLocaleString('en-US', CSTTimeZoneOptions))}));
  }, [])
