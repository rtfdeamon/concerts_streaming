...
export default function ViewShows({title, type}:{title: string, type: string}) {
    ...
    const weekHandler = () => {
      ...
      today = today.toLocaleString('en-US', CSTTimeZoneOptions).split('T')[0];
      lastDayOfWeek = lastDayOfWeek.toLocaleString('en-US', CSTTimeZoneOptions).split('T')[0];
  ...
  const monthHandler = () => {
      ...
      var firstDay = new Date(today.getFullYear(), today.getMonth(), 1).toLocaleString('en-US', CSTTimeZoneOptions).split('T')[0];
      var lastDay = new Date(today.getFullYear(), today.getMonth() + 1, 0).toLocaleString('en-US', CSTTimeZoneOptions).split('T')[0];
    ...
    const todayHandler = () => {
      ...
      today = today.toLocaleString('en-US', CSTTimeZoneOptions).split('T')[0]
      tomorrow = tomorrow.toLocaleString('en-US', CSTTimeZoneOptions).split('T')[0]
