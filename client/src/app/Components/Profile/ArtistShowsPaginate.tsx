...
function Items({ shows }: {shows: IAcceptedShow[]}) {
     ...
     <OptionsModal isOpen={optionsModalIsOpen} setIsOpen={setOptionsModalIsOpen}  id={id} posterUrl={posterUrl} />
        {/* {shows &&  shows.map((a, i) => (
            <div key={i} className={styles.requestWrapper}>
                ...
                <Link className={styles.showLink} href={`/preview/${a.id}`} >Show: {a.title}</Link>
                  <div className={styles.request}>
                  <div className={styles.imageWrapper}>
                      <Image src={a.poster_url} width={250} height={120} alt="showPoster" />
                  </div>
...
export function ArtistShowsPaginate({ itemsPerPage, shows }: {itemsPerPage: number, shows: IAcceptedShow[]}) {
  ...
  const currentItems = shows.slice(itemOffset, endOffset);
  const pageCount = Math.ceil(shows.length / itemsPerPage);

  // Invoke when user click to request another page.
  const handlePageClick = (event: ISelect) => {
    const newOffset = (event.selected * itemsPerPage) % shows.length;
    console.log(
