            ...
            <div key={s.id} className={styles.requestWrapper}>
              ...
              <Link className={styles.showLink} href={`/preview/${s.concert?.id}`} >Show: {s.concert?.name}</Link>
                ...
                <div className={styles.imageWrapper}>
                    <Image src={typeof s.artist_demo_url !== 'object' ? s.concert?.poster_url as string: Women} width={400} height={120} alt="showPoster" />
                </div>
...
export function ArtistShowsPaginate({ itemsPerPage, sessions }: {itemsPerPage: number, sessions: IArtistRequest[]}) {
  ...
  const filteredSessions = sessions.filter(s => s.status === 'accepted')
  return (
    <>
